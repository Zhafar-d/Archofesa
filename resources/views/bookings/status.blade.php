    ...
    <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid gap-8 lg:grid-cols-12">
            ...
            <div class="space-y-6 lg:col-span-8">
                ...
                {{-- Countdown Sisa Waktu Sewa (Muncul saat status siap_huni / dihuni) --}}
                @if (in_array($status, ['siap_huni', 'dihuni']) && $booking->move_out_date)
                    @php
                        $moveOutCarbon = \Carbon\Carbon::parse($booking->move_out_date)->endOfDay();
                        $moveInCarbon  = \Carbon\Carbon::parse($booking->move_in_date);
                        $totalDays     = max(1, $moveInCarbon->diffInDays($moveOutCarbon));
                        $remainDays    = max(0, (int) now()->diffInDays($moveOutCarbon, false));
                        $elapsedDays   = $totalDays - $remainDays;
                        $progressPct   = min(100, round(($elapsedDays / $totalDays) * 100));
                        $isExpiringSoon = $remainDays <= 7 && $remainDays >= 0;
                    @endphp
                    <div class="rounded-3xl border {{ $isExpiringSoon ? 'border-amber-200 bg-amber-50/30' : 'border-emerald-200 bg-emerald-50/30' }} p-6 shadow-sm sm:p-8">
                        <div class="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
                            <div class="space-y-1">
                                <div class="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider {{ $isExpiringSoon ? 'text-amber-700' : 'text-emerald-700' }}">
                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                ...
                                <h3 class="text-lg font-bold text-slate-900">
                                    @if ($isExpiringSoon)
                                        Masa sewa hampir berakhir!
                                    @else
                                        Kamar Anda sedang aktif dihuni
                                    @endif
                                </h3>
                                <p class="text-xs text-slate-600">
                                    Periode: {{ $moveInCarbon->translatedFormat('d M Y') }} — {{ $moveOutCarbon->translatedFormat('d M Y') }}
                                </p>
                            </div>

                            {{-- Countdown Timer --}}
                            <div class="flex items-center gap-2" id="rental-countdown" data-target="{{ $moveOutCarbon->toIso8601String() }}">
                                <div class="flex flex-col items-center justify-center rounded-2xl {{ $isExpiringSoon ? 'bg-amber-900' : 'bg-slate-900' }} px-3.5 py-2 text-white shadow-inner min-w-[54px]">
                                    <span class="font-mono text-lg font-bold tracking-tight" id="rent-days">{{ str_pad($remainDays, 2, '0', STR_PAD_LEFT) }}</span>
                                    <span class="text-[9px] font-medium uppercase tracking-wider text-slate-400">Hari</span>
                                </div>
                                <span class="font-mono text-lg font-bold text-slate-400">:</span>
                                <div class="flex flex-col items-center justify-center rounded-2xl {{ $isExpiringSoon ? 'bg-amber-900' : 'bg-slate-900' }} px-3.5 py-2 text-white shadow-inner min-w-[54px]">
                                    <span class="font-mono text-lg font-bold tracking-tight" id="rent-hours">00</span>
                                    <span class="text-[9px] font-medium uppercase tracking-wider text-slate-400">Jam</span>
                                </div>
                                <span class="font-mono text-lg font-bold text-slate-400">:</span>
                                <div class="flex flex-col items-center justify-center rounded-2xl {{ $isExpiringSoon ? 'bg-amber-900' : 'bg-slate-900' }} px-3.5 py-2 text-white shadow-inner min-w-[54px]">
                                    <span class="font-mono text-lg font-bold tracking-tight" id="rent-minutes">00</span>
                                    <span class="text-[9px] font-medium uppercase tracking-wider text-slate-400">Menit</span>
                                </div>
                                <span class="font-mono text-lg font-bold text-slate-400">:</span>
                                <div class="flex flex-col items-center justify-center rounded-2xl {{ $isExpiringSoon ? 'bg-amber-900' : 'bg-slate-900' }} px-3.5 py-2 text-white shadow-inner min-w-[54px]">
                                    <span class="font-mono text-lg font-bold tracking-tight" id="rent-seconds">00</span>
                                    <span class="text-[9px] font-medium uppercase tracking-wider text-slate-400">Detik</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-5">
                            <div class="flex items-center justify-between text-xs text-slate-600 mb-2">
                                <span>Sudah berjalan <strong class="text-slate-800">{{ $elapsedDays }} hari</strong></span>
                                <span>Tersisa <strong class="text-slate-800">{{ $remainDays }} hari</strong></span>
                            </div>
                            <div class="h-2.5 w-full rounded-full bg-slate-200 overflow-hidden">
                                <div class="h-full rounded-full transition-all duration-500 {{ $isExpiringSoon ? 'bg-amber-500' : 'bg-emerald-500' }}" style="width: {{ $progressPct }}%"></div>
                            </div>
                        </div>

                        {{-- Action Buttons --}}
                        <div class="mt-5 flex flex-wrap items-center gap-3 border-t {{ $isExpiringSoon ? 'border-amber-200' : 'border-emerald-200' }} pt-4">
                            <a href="{{ route('booking.bukti', $booking) }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50">
                                <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
                                </svg>
                                <span>Bukti Reservasi</span>
                            </a>
                            @if ($isExpiringSoon && $status === 'dihuni')
                                <a href="{{ route('customer.extend.form', $booking) }}" class="inline-flex items-center gap-2 rounded-xl bg-[#c9a227] px-5 py-2 text-xs font-bold text-white shadow-sm transition hover:bg-[#b68d1f]">
                                    <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                    <span>Perpanjang Sewa Sekarang</span>
                                </a>
                            @endif
                        </div>
                    </div>
...
<script>
    ...
    // Hitung Mundur Sisa Waktu Sewa (Rental Countdown)
    document.addEventListener('DOMContentLoaded', function () {
        const rentalContainer = document.getElementById('rental-countdown');
        if (!rentalContainer) return;

        const targetDate = new Date(rentalContainer.dataset.target).getTime();
        const elDays = document.getElementById('rent-days');
        const elHours = document.getElementById('rent-hours');
        const elMinutes = document.getElementById('rent-minutes');
        const elSeconds = document.getElementById('rent-seconds');
